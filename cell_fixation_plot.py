...
OUT_PNG = "age_bias_homogeneity_plot.png"
OUT_CSV = "age_bias_homogeneity_plot.csv"
...
NOTE: here, we compute "homogeneity across cells":
- For each cell, define its "cell haplotype" only if that cell is internally fixed
  (all mtDNA sequences in the cell are identical). Otherwise, that cell is "mixed".
- Across the population, compute the fraction of cells that share the most common
  fixed haplotype among fixed cells. Mixed cells do not contribute to the haplotype
  counts (but do contribute to total cell count by default, so homogeneity is
  conservative when many cells are mixed).
"""
def parse_snapshot_homogeneity(path: str) -> Tuple[float, int]:
    #initialize total cell counts
    total = 0

    #store sequences in current cell
    current_cell_seqs: List[str] = []

    #count of fixed-cell haplotypes across cells
    hap_to_count: Dict[str, int] = defaultdict(int)

    #check if cell fixed, and update hap_to_count
    def finalize_cell():
        ...
        #if all sequences in this cell are identical, record the haplotype
        if len(set(current_cell_seqs)) == 1:
            hap = current_cell_seqs[0]
            hap_to_count[hap] += 1
    ...
    #homogeneity across cells:
    #fraction of all cells that are in the most common fixed haplotype
    if not hap_to_count:
        #no fixed cells -> no across-cell consensus
        return 0.0, total
...
mean homogeneity across cells across reps at a given timestep,
...
def compute_series(sim_out: str) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    ...
    #prepare container to store homogeneity from each rep at a given timestep
    ...
    for rep_dir, files in rep_to_files.items():
        ...
        for fp in files:
            ...
            #compute homogeneity, total from snapshot
    ...
    #compute mean homogeneity per timestep
...
def main() -> None:
    ...
    for spec in PLOT_SPECS:
        ...
        #compute homogeneity per simulation
    ...
    with open(OUT_CSV, "w", newline="", encoding="utf-8") as f:
        w = csv.writer(f)
        w.writerow(["label", "timestep", "mean_pct_homogeneity", "std_pct_homogeneity", "n_reps"])
        for s in series:
