...
from init_fasta import InitSpec
...
"""
helper function to do weighted random sampling from log-weights
picks index (int) with probability proportional to exp(logw[i])
"""
def _pick_index_by_logweights(logw: np.ndarray, rng: np.random.Generator) -> int:
    #take in array of logweights and check if empty or not 
    n = int(len(logw))
    if n <= 0:
        raise ValueError("Cannot pick from empty log-weight array.")

    #subtract max logweight from all so that largest is 0 (stabilization)
    lw = logw - float(np.max(logw))
    #convert logweights into weights by exp 
    w = np.exp(lw)
    wsum = float(w.sum()) #sum of weights
    #if weights are broken, pick uniform random index 
    if not np.isfinite(wsum) or wsum <= 0.0:
        return int(rng.integers(0, n))

    #normalize logweights to probabilities
    p = w / wsum
...
def moran_step_cell(
    ...
    #initialize death weights to 0/uniform
    logw_death = np.zeros(n, dtype=float)
    ...
    if cfg.mtdna_death_age_bias != 1.0:
        ...
        #produce logweighted death by adding age x log(bias) for each position
        logw_death += ages * float(np.log(cfg.mtdna_death_age_bias))
    ...
    if mtdna_decay_sel is not None and float(getattr(cfg, "mtdna_decay_sel_strength", 0.0)) != 0.0:
        ...
        #add selection to logweights (+= strength x additive eff)
        logw_death += strength * eff
    
    #pick which mtdna dies by logweights
    dead = _pick_index_by_logweights(logw_death, rng)

    #indices for survivors
    idxs = [i for i in range(n) if i != dead]
    #initialize duplication weights to 0/uniform
    logw_dup = np.zeros(len(idxs), dtype=float)
    ...
    if mtdna_dup_sel is not None and float(getattr(cfg, "mtdna_dup_sel_strength", 0.0)) != 0.0:
        ...
        logw_dup += strength * eff

    #pick which surviving mtdna is duplicated by logweights
    dup_local = _pick_index_by_logweights(logw_dup, rng)
    dup = idxs[dup_local]
...
def apply_mtdna_loss(
    ...
    for cell in pop:
        ...
        for _ in range(k):
            #initialize logweights to 0 across mtdna
            n = len(cell.mtdna)
            ...
            if cfg.mtdna_death_age_bias != 1.0:
                ...
                ages = np.array([m.age for m in cell.mtdna], dtype=float)
                #produce logweighted death by adding age x log(bias) for each position
                logw += ages * float(np.log(cfg.mtdna_death_age_bias))
            ...
            dead = _pick_index_by_logweights(logw, rng)
...
def mitotic_division(
    ...
    while len(replicated) < target:
        ...
        n = len(replicated)
        logw_dup = np.zeros(n, dtype=float)
        ...
        dup_idx = _pick_index_by_logweights(logw_dup, rng)
...
def apply_cell_loss(
    ...
    for _ in range(k_loss):
        ...
        if cfg.cell_death_age_bias != 1.0:        
            ...
            #produce logweighted death by adding age x log(bias) for each position
            logw_death += ages * float(np.log(cfg.cell_death_age_bias))
        ...
        if cell_decay_sel is not None and float(getattr(cfg, "cell_decay_sel_strength", 0.0)) != 0.0:        
            ...
            #add selection to logweights (+= strength x additive eff)
            logw_death += strength * scores

        #pick index for death
        dead_idx = _pick_index_by_logweights(logw_death, rng)
        #kill cell
        pop.pop(dead_idx)
...
def mitotic_generation(
    ...
    for _ in range(ncells):
        ...
        if cell_dup_sel is not None and float(getattr(cfg, "cell_dup_sel_strength", 0.0)) != 0.0:
            ...
            logw_dup += strength * scores

        #pick index for duplication
        parent_local = _pick_index_by_logweights(logw_dup, rng)
