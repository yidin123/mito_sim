...
try:
    ...
    def njit(*args, **kwargs):
        ...
        #this keeps the same code path runnable without JIT acceleration
        if args and callable(args[0]):
            return args[0]
        def deco(fn):
            #deferred decorator form for @njit(...)
...
def _fill_ones_nb(out: np.ndarray, n: int) -> None:
    #neutral baseline helper
    #everything else is basically a perturbation away from uniform weight
    for i in range(n):
        out[i] = 1.0
...
def _fill_mt_age_weights_nb(
    ...
    #mtDNA age weighting lives here
    #age_bias=1 makes age disappear from this part of the model
    if age_bias == 1.0:
        for i in range(M):
            out[i] = 1.0
    else:
...
def _fill_cell_age_weights_nb(
    ...
    #same idea as mtDNA age weighting, but one level up at the cell scale
    #this lets cell age and mtDNA age be biased independently
    if age_bias == 1.0:
        for i in range(n):
            out[i] = 1.0
    else:
...
def _apply_relative_scores_inplace_nb(
    ...
    #selection here is relative to the mean score in the current set
    #so this is more like local competition than absolute fitness
    ssum = 0.0
    cnt = 0
    for i in range(n):
        x = scores[i]
...
def _pick_index_by_weights_nb(w: np.ndarray, n: int, u: float) -> int:
    #weighted sampling by cumulative weights
    #fallback to uniform keeps degenerate weight cases from breaking the run
    total = 0.0
    for i in range(n):
        wi = w[i]
...
def _current_bases_for_positions_nb(
    ...
    #recover current bases only at requested positions
    #this avoids materializing the whole haplotype just to inspect a few sites
    n = positions.shape[0]
...
def _merge_sparse_child_nb(
    ...
    #merge inherited parent mutations with newly generated mutations
    #anything that returns to reference gets dropped so the sparse form stays minimal
    P = parent_pos.shape[0]
    M = mut_pos.shape[0]
...
class SparseHaplotype:
    #sparse haplotype = only deviations from reference
    #score_cache matters because the same haplotypes get queried many times
    pos: np.ndarray
    alt: np.ndarray
...
class HaplotypeBank:
    #central registry for haplotypes
    #population state only needs integer ids instead of full sequences
    def __init__(self, reference: np.ndarray, alphabet: str):
        #store the reference once and treat all haplotypes as sparse deviations from it
        #id 0 is the reference haplotype, which becomes the default baseline state
    ...
    def _key(self, pos: np.ndarray, alt: np.ndarray) -> Tuple[Tuple[int, int], ...]:
        #canonical hashable representation of a haplotype
        #this defines when two sparse haplotypes are treated as identical
        return tuple((int(p), int(a)) for p, a in zip(pos.tolist(), alt.tolist()))

    def register_dense(self, seq: np.ndarray) -> int:
        #convert dense sequence input into the sparse internal representation
        #reference-like sequences collapse directly to haplotype id 0
    ...
    def _register_sparse(self, pos: np.ndarray, alt: np.ndarray) -> int:
        #deduplicate haplotypes before adding anything new to the bank
        #same mutation set should always point to the same haplotype id
    ...
    def materialize(self, hid: int) -> np.ndarray:
        #reconstruct a full sequence from sparse storage
        #mostly useful for output/debugging rather than the core simulation loop
    ...
    def base_at(self, hid: int, pos: int) -> int:
        #random access into one site of a sparse haplotype
        #reference base is the default unless this haplotype overrides it
        hap = self.haps[hid]
    ...
    def _get_mut_cache(self, model: MutationModel) -> Dict[str, np.ndarray]:
        #cache reference-base mutation rates for this mutation model
        #avoids redoing the same length-wide lookup every duplication event
        key = id(model)
    ...
    def _current_bases_for_positions(self, hid: int, positions: np.ndarray) -> np.ndarray:
        #small Python wrapper around the numba sparse lookup
        #fast paths handle empty/reference haplotypes without extra work
        if positions.size == 0:
    ...
    def _child_from_parent_with_mutations(
        ...
        #construct the child haplotype after mutation hits are chosen
        #the result may still collapse back to an existing haplotype id
        if mut_pos.size == 0:
            return hid
    ...
    def mutate_child(
        ...
        #mutation is applied relative to the current haplotype state
        #so the transition process is state-dependent rather than just reference-centered
        if model is None:
            return hid
        ...
        #once mutation hits are known, recover the actual current bases at those sites
        #this matters because a site can already differ from reference
        ...
        #each hit samples a new base from the position/base-specific transition kernel
        #so mutation is local but not necessarily symmetric
    ...
    def _ref_score(self, sel: SelectionModel, cache_key: str) -> float:
        #baseline additive score of the reference sequence
        #cached because every non-reference haplotype is scored as reference plus deltas
    ...
    def hap_additive_score(self, hid: int, sel: SelectionModel, cache_key: str) -> float:
        #additive fitness score for one haplotype
        #only mutated positions need to be evaluated relative to reference
        hap = self.haps[hid]
    ...
    def score_many_into(
        ...
        #batch score haplotypes into a preallocated output array
        #optimized for repeated ids because many mtDNA copies often share haplotypes
        n = int(hids.size)
        if n == 0:
            return out[:0]
    ...
    def score_many(
        ...
        #convenience wrapper when no preallocated output buffer is supplied
        #less allocation-conscious than score_many_into but simpler to call
...
@dataclass
class Population:
    #main mutable state of the simulation
    #mtDNA-level state and cell-level state are stored side by side
    copy_hid: np.ndarray
    mt_birth: np.ndarray
    m_count: np.ndarray
    cell_birth: np.ndarray
...
def deterministic_counts(n: int, probs: np.ndarray) -> np.ndarray:
    #turn proportions into exact integer counts without initialization noise
    #largest fractional remainders get the leftover counts
...
def _pick_index_by_weights_prefix(w: np.ndarray, n: int, rng: np.random.Generator) -> int:
    #rng-facing wrapper around the numba weighted sampler
    #keeps random number generation outside the compiled helper
    return _pick_index_by_weights_nb(w, n, float(rng.random()))


def mt_ages(pop: Population, ci: int) -> np.ndarray:
    #derive mtDNA ages from simulation time and birth times
    #age is implicit in the birth timestamp rather than updated every step
    M = int(pop.m_count[ci])
    return pop.sim_time - pop.mt_birth[ci, :M]


def cell_ages(pop: Population) -> np.ndarray:
    #same age convention at the cell level
    #useful for inspecting the population state without changing it
...
def _fill_scores_for_cell(
    ...
    #score all mtDNA copies inside one cell
    #writes into a reusable scratch buffer to avoid repeated allocation
    M = int(pop.m_count[ci])
...
def _recompute_cell_sum(
    ...
    #cell-level score is treated as an aggregate of its mtDNA scores
    #this is where within-cell haplotypes become a cell-scale quantity
    if sel is None:
        return 0.0
    M = int(pop.m_count[ci])
    hids = pop.copy_hid[ci, :M]
...
def initialize_population_multi_fast(
    ...
    #build initial population from deterministic cell-type proportions
    #initial randomness is mostly just shuffling cell order after templates are made
...
def maybe_transfer_event(pop: Population, cfg: SimConfig, rng: np.random.Generator) -> None:
    #optional mtDNA exchange between pairs of cells
    #this introduces mixing that is not explained by ordinary lineage descent
    ...
    i = 0
    #walk through shuffled cells and form transient exchange pairs
    #this makes transfer sparse rather than a global mixing event
    while i < n - 1:
        a = int(available[i])
        if rng.random() >= p:
            i += 1
        ...
        kk = min(k, na, nb)
        if kk > 0:
            #swap selected mtDNA copies between the two cells
            #so copy number stays unchanged while cellular context changes
...
def moran_step_cell(
    ...
    #one mtDNA-level Moran turnover step inside a cell
    #death and duplication are balanced, but each side can have age/selection bias
    M = int(pop.m_count[ci])
    if M <= 1:
        return
    ...
    #choose the mtDNA copy that is removed first
    #this can be uniform, age-biased, selection-biased, or both
    ...
    #choose the parent copy for duplication while excluding the copy that just died
    #this is the conservation step that keeps mtDNA count stable
    if not use_dup_sel:
    ...
    #duplication partially refreshes the parent age
    #so age is a lineage-like quantity rather than only chronological time
    new_parent_age = parent_age * float(cfg.mtdna_dup_refresh_parent)
    pop.mt_birth[ci, parent] = pop.sim_time - new_parent_age

    #the child can mutate during duplication and can inherit a scaled version of parent age
    #this keeps mutation and age inheritance coupled to replication events
...
def moran_step_population(
    ...
    #apply one mtDNA turnover step to every cell
    #cells are independent here except through shared bank/config objects
...
def apply_mtdna_loss(
    ...
    #remove mtDNA copies without immediate replacement
    #this lets copy number decrease while still using the same death-bias machinery
    if k_loss <= 0:
        return
    ...
    #apply loss independently within each cell
    #the active mtDNA count shrinks by moving the last copy into removed slots
    for ci in range(pop.n_cells):
        M = int(pop.m_count[ci])
...
def apply_cell_loss(
    ...
    #remove whole cells from the active population
    #same general idea as mtDNA loss, but now the unit is a cell state
    if k_loss <= 0:
        return pop
    ...
    #each loss event removes one active cell by compacting the last cell into its slot
    #this keeps arrays fixed-size while n_cells tracks the active prefix
...
def mitotic_division(
    ...
    #cell division is modeled as mtDNA amplification to 2x followed by random partition
    #this makes division both a replication step and a drift-generating split
    ...
    #amplification phase: repeatedly duplicate mtDNA until the pool reaches 2x
    #selection, if enabled, biases which copies contribute to that expansion
    while cur < target:
        if use_dup_sel:
            w = scratch_w[:cur]
            _fill_ones_nb(w, cur)
    ...
    #shuffle before splitting so daughter assignment is not an ordering artifact
    #the split itself is where division-level drift enters
    ...
    #cell age is also inherited with asymmetric refresh factors
    #so daughter cells can immediately differ in age state
...
def mitotic_generation(
    ...
    #cell-level Moran-like generation
    #a cell dies, another divides, and mtDNA dynamics are coupled into the replacement event
    ncells = pop.n_cells
    if ncells <= 1:
        return pop
    ...
    #each iteration is one cell death plus one compensating division event
    #so population size stays fixed even while identities turn over
    for _ in range(ncells):
        pop.sim_time += float(cfg.mitotic_age_step)
        n = pop.n_cells

        #choose which cell is removed from the population
        #death can be neutral, age-biased, selection-biased, or both
        if age_bias == 1.0 and not use_cell_decay:
        ...
        #choose which surviving cell divides to fill the vacancy
        #cell duplication selection acts here, separate from mtDNA-level selection
        if not use_cell_dup:
            if n == 2:
        ...
        #division produces two daughter states: one replaces the dead cell, one replaces the parent
        #this is where between-cell turnover and within-cell mtDNA replication meet
