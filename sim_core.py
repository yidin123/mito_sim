...
used in case of no heteroplasmy thresholding
"""
def _cell_effect_sum_additive(cell: Cell, sel: Optional[SelectionModel]) -> float:
    #no selection model provided
    if sel is None:
        return 0.0
...
"""
helper function for additive effects of mtdna base pairs on cell-level IF thresholding is applied
for each position, we first compute WT/neutral fraction = (# mtdna with effect == 0 at that position) / (total mtdna copies)
then sum effects ONLY for non-neutral variants whose threshold > wt_frac
returns score in teh same scale as cell_effect_sum_additive (divided by 1e6 scaling factor)
"""
def _cell_effect_sum_thresholded(cell: Cell, sel: Optional[SelectionModel]) -> float:
    if sel is None:
        return 0.0
    if len(cell.mtdna) == 0:
        return 0.0
    if len(cell.mtdna[0].seq) != sel.length:
        raise ValueError(f"seq length {len(cell.mtdna[0].seq)} != selection length {sel.length}")

    #enforce that if thresholding is used, any nonzero effect encountered must have a threshold
    #(otherwise behavior is ambiguous).
    total = len(cell.mtdna)

    #only positions that appear in effects/thresholds can matter; everything else is neutral.
    #effects includes zeros too because the file usually provides per-base entries.
    positions = {pos0 for (pos0, _b) in sel.effects.keys()}
    #if someone only provided thresholds for a subset, include those too:
    positions |= {pos0 for (pos0, _b) in sel.thresholds.keys()}
    ...
    for pos0 in positions:
        ...
        #first pass: count neutral at this position
        for m in cell.mtdna:
            base = m.seq[pos0]
            eff = sel.effects.get((pos0, base), 0.0)
            if eff == 0.0:
                neutral += 1

        wt_frac = neutral / float(total)

        #second pass: add gated effects
        for m in cell.mtdna:
            base = m.seq[pos0]
            eff = sel.effects.get((pos0, base), 0.0)
            if eff == 0.0:
            ...
            thr = sel.thresholds.get((pos0, base), None)
            if thr is None:
                raise ValueError(
                    f"Thresholding is ON but missing threshold for pos={pos0+1}, base={base} (effect={eff}). "
                    "Use 'effect,threshold' in the selection file for that base."
                )

            #contribute if variant threshold is ABOVE current WT/neutral heteroplasmy
            if float(thr) > float(wt_frac):
                score_raw += float(eff)

    #match scaling used in _mtdna_effect_sum (divide by 1e6)
    return float(score_raw) / 100.0

"""
helper function to generally get cell selection scores
"""
def _cell_effect_score(cell: Cell, sel: Optional[SelectionModel], cfg: SimConfig) -> float:
    if getattr(cfg, "thresholding", False):
        return _cell_effect_sum_thresholded(cell, sel)
    return _cell_effect_sum_additive(cell, sel)
...
def apply_cell_loss(
    ...
    for _ in range(k_loss):
        ...
        if cell_decay_sel is not None and float(getattr(cfg, "cell_decay_sel_strength", 0.0)) != 0.0:
            ...
            scores = np.array([_cell_effect_score(c, cell_decay_sel, cfg) for c in pop], dtype=float)
            #apply multiplicative linear fitness factor
...
def mitotic_generation(
    ...
    for _ in range(ncells):
        ...
        if cell_dup_sel is not None and float(getattr(cfg, "cell_dup_sel_strength", 0.0)) != 0.0:
            ...
            scores = np.array([_cell_effect_score(pop[i], cell_dup_sel, cfg) for i in idxs], dtype=float)
